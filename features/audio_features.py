...
LastEditTime: 2021-09-13 00:05:16
'''
import torch
...
class AudioFeatureExtractor_KE(BaseFeatureExtractor):
    def __init__(self, config):
        super().__init__()
        print("Initializing AudioFeatureExtracor...")
        self.feature_dim = config["audio"]["feature_dim"]
    ...
    def get_feature(self, clip, target_character):
        ...
        for ii, speaker in enumerate(speakers): # seq_len, n_c
            ...
            ret_concepts.append(torch.LongTensor(convert_examples_to_ids(self.concepts[index] if index in self.concepts else [], self.concept2id)) ) # seqlen
