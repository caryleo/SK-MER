...
LastEditTime: 2021-11-14 13:38:45
'''
import json
import pandas as pd
import importlib
...
def create_model(config):
    print("Creating Model", "[" + config['model']['type'] + "]", "...", end=' ')
...
def create_dataloader(config):
    ...
    print("Creating Dataloader", "[" + config['data_loader']['type'] + "]", "using Dataset", "[" + config['data_loader']["dataset"] + "]", "...", end=' ')
...
def create_trainer(model, criterion, metrics, logger, config, data_loader, valid_data_loader):
    ...
    print("Creating Trainer", "[" + config['trainer']['type'] + "]", "...", end=' ')       
    # print("trainer [%s] was created" % (config['trainer']['type']))
    print("Done!")
    logger.info(model)
    return instance

def create_trainer_cl(model, criterion, metrics, logger, config, data_loader, valid_data_loader, criterion_cl):
    ...
    print("Creating Trainer", "[" + config['trainer']['type'] + "]", "For CL ...", end=' ')       
    # print("trainer [%s] was created" % (config['trainer']['type']))
    print("Done!")
    logger.info(model)
    return instance
